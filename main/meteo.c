// Based on dnoas meteo.c

#include "meteo.h"

char* TAG_METEO = "TASK_METEO";

bmp180_dev_t dev_descriptor;

void meteo_init() 
{
    ESP_LOGI(TAG_METEO, "Initializing BMP180 meteo module");
...
void task_meteo(void *args) 
{
    ...
    ESP_LOGI(TAG_METEO, "BMP180 meteo module reports %f C and %u hPas ", temperature, pressure);

    ESP_LOGI(TAG_METEO, "Waiting for NTP");
    xSemaphoreTake(mqtt_semaphore, portMAX_DELAY);
    ESP_LOGI(TAG_METEO, "NTP ready. Starting read loop");
    ...
    while(true) 
    {
        ...
        ESP_LOGI(TAG_METEO, "Queued sample MeteoData %f C and %u hPas", temperature, pressure);
    }
}